...
5. ~~**吸顶泳道**：工具栏固定在阅读列顶部，每轮的过程状态行固定在其下方。~~
   **已在 0.3.0-relayout.6 整体回退**：工具栏与状态行都回到正常文档流，配套的
   `--reader-toolbar-height` 实测与为 `sticky` 覆盖的 `overflow` / `position` 一并移除。
   （`.jumpDock` 与时间轴轨道自己的 `sticky` 是上游原有的，未动。）

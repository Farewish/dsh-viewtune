...
两条限制都不会再以**静默消失**的方式呈现。数据不给数字，界面就说明为什么——一枚虚线的灰标记，
悬停即可看到原因：

- **被历史窗口截断的轮次**不显示步骤**数字**。Harness 按消息条数分页，最上面那一轮可能只加载了一部分，
  它的步骤数是局部和，与过程记录里的绝对步数不可比——偏小的分母会误导，所以不显示。
  取而代之的是 **「步骤记录 · 窗口外」** 标记（悬停说明原因，并提示用「加载更早记录」把它载入）。
- **用量偶尔缺数字**属于宿主的有意为之：它只在能**精确证明**该轮计费数据时才给出用量（例如该轮某次尝试
  没有可用的用量样本，或该轮落在压缩上下文里）。本插件不猜、不估算，但也不再什么都不显示：
  轮次结束后若拿不到用量，动作行里会出现 **「用量 —」** 标记，展开后的浮层里写明原因。

两次标记都只是**说明**，不是控件：没有点击行为、没有 hover 高亮，也不会出现任何估算出来的数字。

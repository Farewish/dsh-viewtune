...
- **工具栏**：「收起」+「viewtune ⚙」在阅读视图顶部一条泳道里，**横跨整个阅读视图**——分隔线与宿主的顶部栏一样从一头到另一头，而两个控件仍停在文字左边缘。它**吸顶在顶部栏正下方**——属于外层的横条，不是浮在对话内容上的东西，所以滚到任何深度都够得到；快捷键 **Alt+C** / **Alt+Shift+C** 照旧。
- **壁纸**：设置「视效」页的「壁纸」一行让你拿自己的一张图当阅读视图的底。图片放在插件自己的文件夹里（点「打开文件夹」会建好并打开它，把图拖进去再点「刷新」就变成缩略图列表），点一张即生效，「清除」回到不设。**浏览器不知道这个文件夹在哪**：它只按名字要图，宿主那半只在那个文件夹里、且只对图片扩展名作答（不含 svg）。旁边的「压暗」滑块把图混向主题底色，好让正文压在上面也看得清；开着「磨砂玻璃」时卡片糊的正是这张图。壁纸**默认铺满整个窗口**——转录区、它下面的空处、滚动条槽位、以及输入框上方那条**淡出带**（那条是把宿主的不透明渐变改成「遮罩 + 同一份底图」的，所以内容照样平滑淡出，只是淡入的从一块底色变成你的图），因此这些地方不会留成黑块。只覆盖阅读视图那一列时，图片**填满阅读页面**：按"图片 ÷ 目标"的**最大值**取比例（也就是"不留未填充区域所需的最小比例"），以阅读页面为基准居中裁掉溢出；**小图会放大去填满**（没有放大上限）。**「铺满整个窗口」默认开着**：它把图蔓延到**左侧栏与顶部栏**（那两栏原本的底板被让开），此时改为 `cover` 填满整窗——也就是说这个开关管的是"**要不要蔓延到别的列 / 要不要填满**"，而不是"要不要铺"。打开后多出的**「界面遮罩」**滑块决定那两栏压在自己那份壁纸上的浓度——它们上面全是文字，`0%` 就是直接压在照片上。图只画一次、遮罩按窗口算一层，所以阅读区不会比旁边更暗。
- **设置面板**：点「viewtune ⚙」打开一个小面板，分「视效」「功能」「快捷键」三页（键盘用方向键 / Home / End 在页间切换）。「视效」里偏好一项一行——「动效」（被系统的减少动态效果设置覆盖时会说明）和「磨砂玻璃」（默认开着——出厂默认就是这个读者的设置；打开后工具栏、卡片、工具框与代码块都不再自带不透明底板，透出宿主壁纸与皮肤，路径/行数这类标签静止时透明、悬停或聚焦才显出轮廓。**开关下面九个滑块**分别调各面不透明度：工具栏 / 用户气泡 / 卡片与面板 / 代码块 / 差异面板 / 滚动条槽位 / 产物标签 / 用量与步骤胶囊 / 输入框——其中「滚动条槽位」是给最右侧那条滚动条的轨道加的一条「道」，0% 就是宿主原本的透明轨道），`0%` 是「这个面什么都不画」，`100%` 是关掉皮肤时那块不透明底板，出厂值即是这个读者自己调出来的那个外观。分组按"是什么"而不是"在哪"（差异纸无论画在面板里还是工具「结果」页里，都归「差异面板」），而且**只给本来就有底板的面配旋钮**：「产物标签」只管一轮末尾那栏产物文件（它是唯一常驻、静止就带底的小标签），压缩胶囊归「卡片与面板」，而「用量 … tok / … 个步骤」那两枚计数胶囊有自己的旋钮，工具状态与行数计数在基础样式里本没有静止底板，所以不接任何旋钮）；「功能」页放着**「竖条滚轮」**（默认开：阅读列两边那两条竖条上的滚轮会转发给正文）与「产物用右侧栏打开」（默认关闭：点产物文件用系统程序打开；打开后改用右侧栏的内置预览，走的正是官方那套 `dsh-resource://file/session/<id>/<path>` 地址与侧栏服务，文件夹与「在文件夹中显示」仍走系统程序；宿主没有这个服务时自动回落系统程序并在控制台说明）；「快捷键」页可以改本插件自己的两个快捷键（点键帽录制新组合，Esc 取消，清除即不带快捷键），会拒绝没有修饰键或已被浏览器占用的组合。加一个偏好就是加一行，加一类就是加一页，工具栏不会因此变宽。只有标签说不清的行才有说明，那句说明挂在**该行的 `title` 上**（浏览器弹的原生框，和按钮、「收起」是同一套），平时不占版面；而被系统覆盖、录制中、组合被拒这类**状态**仍然直接写在行里。
...
轮次渲染顺序、§ 收起开关的作用域与淡出、工具栏形状、步骤胶囊用到的标识符是否都有声明，

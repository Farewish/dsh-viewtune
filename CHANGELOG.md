...
**B1：两处"静默消失"改成显式标记。**
- 被历史窗口截断的轮次不再什么都不显示，而是给出 **「步骤记录」** 标记（虚线灰标记，
  悬停一句 **该轮未加载完全**）。**仍然不显示任何数字**——偏小的分母会误导。
- 轮次结束后若宿主未能给出可证明的用量，动作行里出现 **「用量 —」** 标记（悬停：宿主未提供该轮用量，未估算），
  它就在「步骤记录」左边；两者各自独立出现，用量可证的轮次不会显示这枚标记。同样不估算、不给数字。

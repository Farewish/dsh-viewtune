...
**修复：交棒判定没有余量，导致卡片到底后要多滚一格才把滚动交出去。** 上一条我用的判据是**精确相等**：
...
而 `port.scrollTop` 是**小数**——原生滚动有子像素累积。卡片看起来停在底部时，实际往往还差零点几像素，于是那次滚动被"卡片其实还能动"吃掉（卡片原地不动），**交棒要等到下一次滚动**才发生。表现就是滚到底后发涩、然后才跳到页面——而它只在**长**思考出现，因为只有长卡片会真的滚到底。

现在抽出具名函数 `atScrollEdge(scrollTop, delta, maxOffset, slack = 1)`，**余量 1px**：足以吃掉子像素误差，又小到读者察觉不到"少滚了一点"。提问方式也变了——不是"位置是否等于边界"，而是"**这次滚动还能不能移动卡片**"（对投影做 clamp 正是浏览器自己的做法），于是上和下两个边界由同一个表达式覆盖。余量作为参数暴露，需要时可调。

`test-follow-rules.mjs` 扩到 **22 条**，专门盯这个数：**差 1px 算到底、差 2px 不算**——后者是反方向的失效（还没滚完就交棒），所以余量不能调大。并有一条喂进**零余量**的坏实现，确认检查器能识别这个形状（先前就是它悄悄通过了）。

**同一处还修了测试自身的两个问题**：`test-wheel-handler` 的沙箱没有 `atScrollEdge`（函数独立后处理器会以未定义名运行），以及它有一条断言仍写着"零余量时由浏览器补上那 0.5px"——那是旧行为的断言。

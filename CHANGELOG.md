...
**新增三项设置，都在「功能」页：「正文更新节奏」两选一（**默认「固定 60 次/秒」**）、「逐词显现的模糊」开关与「逐词显现」开关（这两项默认保持现状）。**
...
- 三条设置各自的断言（选项数组与其顺序 / 默认即固定 60 次/秒 / 回退方向 / 发布闸门与"隐藏页仍立即 flush" / 关闭模糊时无 filter / 动画确实用了这个选择 / 逐词关掉时两处闸门都在 / 各项防御式读取）与反向自检脚本 `_tmp/negcheck-shape-markers.mjs`（表驱动，七个用例）：把产物回退成旧形状或写回陷阱形状 ⇒ 对应断言全部失败 ✓，产物逐字节还原 ✓。

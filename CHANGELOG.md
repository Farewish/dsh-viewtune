...
- **同一份 `meta.diffs`、两个归一化器（§3.6）**：行上的 +N/-M 角标走 `callDiffHunks`——它把**缺少** `oldText` 读作"没有旧的一侧"（新建文件），并且跳过它用不了的条目；而「结果」页有**自己的**一份，遇到同样输入会**整条列表作废**。于是宿主给 `meta.diffs` 写一条省略 `oldText` 的记录时（同一个形状、少一个键），**行上广告了 +N/-M，点开却是通用兜底**——正是 M4 那个"行上有、展开没有"的同一类错，只是换了个字段 ✗⇒✓。现在两个消费方都走 `diffHunksOf()`（导出、也有测试），一条用不了的记录**被跳过而不是连坐**它旁边的文件，而"整条都不可用"回答 `null`（调用方问的是"这到底是不是差异面"），不是真值空数组 ✓。守护一条 marker（含"第二份归一化器不许回来"这一反向断言）+ 一个反向用例 ✓。
- **`listWallpapers` 逐个 `stat`（§5.5）**：几百张图的目录就是几百次串行 await，每次只为一个列表本来带不上的数字。改成 `Promise.all`，结果按**列表顺序**收集、之后再排序，所以答案不取决于哪次 stat 先回来 ✓。
- **`readBody` 先拼字符串再 `Buffer.byteLength`（§5.5）**：每个 chunk 都重新编码**整段已收内容** ⇒ 一个正被拒绝的超大请求同时也是最贵的一个（对 chunk 数是平方级）。改成只累加本 chunk 的字节数：超限时立刻回答、后续 chunk 直接丢弃 ✓。
- **`wallpaper-files.ts` 里两条语句挤在一行**的格式瑕疵一并修掉（就是上面那个函数的签名行）✓。
- **§5.1 / §5.2 的代价写进 README（两个语言版本）**：整条覆盖写没有版本号/时间戳 ⇒ **两个窗口同时改设置是后写静默覆盖先写**；启动读记录返回前的几百毫秒里做的改动**发不出去、且会被随后到达的记录回滚**（同一个顺序还让 `hydrate` 立刻把刚读到的记录再 PUT 一遍）。两条都**保留**：要修得先引入版本号与冲突策略、或把"读者刚改的"与"来自记录的"分开记账，那是比它们保护的东西更复杂的机器，而窗口只有几百毫秒。**审计要求的正是"把代价写下来"，所以写下来了** ✓。
- **`data-*` 观测面有了据可查的理由**：审计说某批 `data-*`"全仓库无消费者"。核实后写进 README——插件自身确实不读它们，但工作区那批脚本读（`render-dump.mjs`、`feature-matrix.mjs`、`verify-patched-instance.mjs`、`probe-linked-host.mjs`、`marker-scan.mjs` 等，逐个 grep 过），它们是"跑一次真实页面看状态机走到哪"的唯一观测点 ⇒ **不删**，并说明为什么不删 ✓。
- 验证：build、`tsc`、**35/35** 测试文件、**22/22** 不变式、**55/55** 反向用例全过 ✓。
...
- **M11 的其余性能项**：壁纸 effect 在**视图作用域**下拖「压暗」会整段重跑（新建 `Image`、重挂 `ResizeObserver`）；M11.5 推理卡动画 effect 每次交互重建；M11.6 `onScroll` 里跑完整 `measure()`；M11.8 非 passive 的 wheel 监听；`resizer-wheel` 每帧全文档查询；`composer-wheel` 每个 wheel 事件强制布局；`inputFields` 在长 `write` 上的 O(n²)；M11.11 轮次级四个 `useChat` 选择器；M11.12 `WordTimeline.births` 不截断。都确认存在，都没动。
- **M8**（挂载时已有文本的那一批文字不做逐词动效）：审计自己已经把它从"每轮都吞第一批"降级为受限情形（真正的流式首帧是空串，所以正常路径不触发）⇒ 留原样、不修。
- **§8 的四条疑似（S1/S2/S3/S5）**：滚动 spy 是否可能持有脱离文档的行、`hasProcess` 是否滞后于内容增量、长空白是否让 `WordTimeline` 的 `settled` 游标卡住、以及 `MutationObserver` 是否被插件自己写 `html/body` 属性触发而给每个 frame 广播。审计说这些**需要运行期实测**才能定论，我没有为它们造假证据 ⇒ 原样留下，结论待探针。
- **§3.2 的两处死值**（`--glass-scrollbar` / `--glass-input` 写在阅读根上、却只从 `<html>` 读，值相同、不影响渲染）、`entry-policy` 的 `0.1.0-trial` 遗留分支（删它要连它的测试一起删，而它保护的是一次旧构建的入口）、`dsh.client.inject` 对构建无效、`lib/client.js.map` 进包、格式脆性与重复断言：都确认了，都没改。
- **`background-attachment: fixed` 不是"可避免的重绘压力"，这条建议不成立**：审计把它列进 §3.1 建议去掉，但这份仓库自己的记录（本文件下面几节）写明它是**承重**的——顶部栏、侧边栏与阅读页各画一份底图副本，正因为 `fixed` 把定位基准放在**视口**上，几张副本才能严丝合缝拼成一张图；去掉就会错位。**不改**，并把这条判读记在这里，免得下次又被当成冗余。`.toolRaw` 的旋钮归属这次**改了**（从卡片组里移出，见上一节）✓。

**生效范围**：客户端半边由 HMR 立即生效；Host 半边的改动（`/reveal` 上限、`readBody` 无论何种情况都落地、设置写盘失败回 500、壁纸 `no-cache` + `ETag`）**需要重启 Host** 才生效 ✓。

## 0.4.1 (Option+C works on macOS, and the markers stop depending on this checkout's path)

**两条都是"在一台机器上永远看不见"的缺陷：一条只在 macOS 上失效，一条只在你把仓库放在别处时才炸。**

- **macOS 上 Option 是系统自己的死键修饰键**：Option+C 报的不是 `key: 'C'`，而是它组合出来的字符 `ç`（Option+Shift+C 是 `Ç`），于是出厂默认 `Alt+C` 在那台机器上**永远不匹配**——而**重新录制却能成功**，因为它存下的是 `Alt+Ç`。这是最难堪的一种形态：面板写着「已绑定」，按键毫无反应，重新录一次看起来就好了 ✗⇒✓。
- 修法读**物理键**：`event.code`（`KeyC`）不受布局组合影响，所以它只在「组合出来的字符不可能是绑定的一部分」（不在 A–Z / 0–9）**且**事件带 `Alt`/`Meta` 时才作为回退。两条边界都刻意收窄：没有修饰键时本来就不是绑定，而一个布局里**真的有** `ç` 键（法语 AZERTY 在 `Digit9`）的读者必须继续匹配他录下的那个键；录制与匹配走**同一个函数**，所以录下来的一定还能被听见 ✓。代价写在这里：那位 AZERTY 读者若曾录过 `Alt+Ç`，重录会变成 `Alt+9`（物理键），可重新绑，罕见，所以选择写在代码里而不是绕开 ✓。
- **四条 marker 把 lightningcss 的类名哈希写死了**（`.WGoHxG_reasonCard…` 等）：这个哈希由 CSS 文件的**绝对路径**决定，所以在**别的目录**里克隆、按文档跑一次 `npm run build`，产物哈希就变了，这四条会为一个与它们守卫的决定毫无关系的理由失败 ✗⇒✓。现在这些选择器由 `sel()` 从**产物自己的样式表**里取类名，`@media` 里的那条改用新的 `atRuleBody()`（按花括号计数取一段 at-rule 的内容，这样"在 no-preference 里"仍然是被断言的，而不是"样式表某处有这条声明"）。
- **这条不再是推理，而是每次构建都测一次**：`verify-build` 本来就在**临时目录里**重新构建一份产物，现在它还把**整套 marker 检查跑在那份产物上**，并把两个哈希都打印出来。实测：本目录 `WGoHxG`，临时目录 `_719uiG`——同一份源码、同一个编译器、两个目录，两个哈希；而检查器在那边**通过**（`exit 0`）✓。反向自检表里的对应四条也改用 `%SEL:local%` 记号，由产物解析出类名；解析不出来的记号会原样留在用例里 ⇒ 那个用例**大声 SKIP**（算失败），而不是去改一段它没打算改的东西 ✓。
- 守护：新增一条 marker（物理键回退的两段形状）与一个反向用例（把带修饰键的判断改成 `false`）✓；反向自检表 52 → **53** 例 ✓。
...
- 读者的原话：卡片底部显示不完整、此时**不出现**「回到最新」、手动滑一下它**反而**出现、点完之后卡片底部**刚刚好完整**、而那时**还能再往下滑一点** ✓。
- **最后两句就是答案**：「回到最新」做的是把滚动写到**最大值**。点完是"刚刚好完整" ⇒ 之前的不完整就是**离真正底部差一个缺口**。缺口为什么一直在：焦点授予的前提是 `isNearTail`，它允许 **72px** 容差；而卡片持有焦点期间页面的跟随是**刻意挂起**的 ⇒ **没有人会来关这几十像素**，于是卡片的底边（底栏与它上面最后几行）就一直低那么多，落进输入框上沿与它那道渐变带里 ✗⇒✓。也解释了为什么没有药丸：缺口不是"读者滚走了"，`following` 一直是 true ✓。
- **改法**：在焦点**授予**的那个 layout effect 里（**绘制之前**）①先把滚动写到最大值——关掉缺口，而授予的前提本来就是"读者在底部"，这是唯一一个"关掉它正是读者所求"的时刻；②再跑一次补偿，因为同一个 commit 里视口上限从 `224px` 变成 `min(60vh, 560px)`，卡片**已经长高**，放到"下一次 measure"就会多画一帧悬在折线以下的卡片。补偿函数在跟随器那个 effect 里，所以通过 `compensateNow` 这个 ref 交出来（这个文件本来就用同样方式交出 `stopFollow`），拆卸时一并清掉 ✓。
- 两次修复是同一件事的两半：一个让卡片的**底边对所有增长（含底栏）保持锚定**，另一个保证**锚定的位置是看得见的那个** ✓。
- **一个被否掉的选项，留档以免日后重来**：给"授予那一跳"加缓动。它看起来顺理成章（滑行模式下其它增长都缓动了），但**单独缓动补偿是不可能的**——补偿必须逐帧等于渲染出的增量，高度瞬间跳而滚动慢慢追会让底边先下滑再追回来；要缓动就得连**高度**一起缓动，那要么给 `max-height` 加过渡（于是**窗口缩放**时 `60vh` 每帧变化会变成卡片高度发飘），要么在绘制前把视口高度**钉住**、再让第一次写入落进已有过渡里。读者权衡后选择不做 ⇒ 保留瞬时的那一跳，只保留这次"关缺口 + 绘制前补偿" ✓。
...
- 底栏（`可滚动阅读 / 暂停跟随 / 展开阅读`）是卡片的一部分，**卡片一开始溢出它就出现**——那一次高度变化发生在视口之外、也发生在写入之外，于是没有任何补偿；而聚焦期间页面的尾部跟随是**刻意挂起**的 ⇒ 卡片的底部（那一行，以及它上面最后几行）被推到可见区之外，且没有东西把它带回来 ✗⇒✓。
- 同一个洞还盖住了更大的一跳：**焦点被授予的那一帧**，视口的上限从 `224px` 换成 `min(60vh, 560px)`——一个 commit 里几百像素的卡片高度，同样一分钱都没补偿 ✗⇒✓。
- 改法是补偿**整个卡片**的高度（`port.closest('[data-reader-reasoning-card]')`），而不是它写入的那个视口；并且在**每次 measure** 都记录并补偿差量——底栏是 React state、由 `measure` 自己设置，所以晚一个 commit 才落地；授予那一跳则在更早的 commit 里 ✓。
- 授予那一跳要算得出来，就必须在**请求焦点之前**记下卡片当时的高度：那是唯一还存在的"授予前高度"（我们的 effect 运行时卡片已经长高了）⇒ 两处授予点都在 `onFocusChange(..., true)` 之前先 `recordCardHeight()` ✓。没有焦点的卡片照旧**只记录、不滚动**，因为记录值必须在授予那一刻是"授予前"的那个数 ✓。

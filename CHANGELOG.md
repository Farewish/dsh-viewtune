...
**新增七项设置，都在「功能」页：「正文更新节奏」两选一（**默认「固定 60 次/秒」**）、「逐词显现的模糊」开关、「逐词显现」开关、「跟随到最新」两选一、「自动收起更早流程」开关、「思考卡自动跟随」三选一、「自动滚动速度」四选一（除节奏外默认都保持现状）。**
...
- 七条设置各自的断言（选项数组与其顺序 / 默认即固定 60 次/秒 / 回退方向 / 发布闸门与"隐藏页仍立即 flush" / 关闭模糊时无 filter / 动画确实用了这个选择 / 逐词关掉时两处闸门都在 / 跟随方式两处闸门都在 / 自动收起的两处问法与清选择闸门 / 思考卡三模式与整行量化 / 模式与行速进了依赖里 / 各项默认与防御式读取）与反向自检脚本 `_tmp/negcheck-shape-markers.mjs`（表驱动，十一个用例）：把产物回退成旧形状或写回陷阱形状 ⇒ 对应断言全部失败 ✓，产物逐字节还原 ✓。

...
- **Wait clock**: while the model is thinking or writing, the status line shows how long this turn has been in its hands — nothing for the first three seconds, since a pause is usually over before a number could be read — and past ten seconds a 「暂未响应」 badge. The anchor is the **last handover** — a returned tool, an injected context, a finished command, your own message — never the start of the turn, so a wait that has just begun does not inherit the minutes the tools already spent; a tool that is still running is not a wait, because then the tool is the one working.
